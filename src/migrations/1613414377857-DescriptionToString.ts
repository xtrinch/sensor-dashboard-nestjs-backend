...
export class DescriptionToString1613414377857 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "topic" ALTER COLUMN "description" TYPE VARCHAR`,
    );
    await queryRunner.query(
      `ALTER TABLE "comment" ALTER COLUMN "description" TYPE VARCHAR`,
    );
  }
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE "comment" ADD "description" jsonb NOT NULL`,
    );
